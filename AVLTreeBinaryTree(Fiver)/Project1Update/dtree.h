...
private:
	...
	bool insertRec(DNode *&Current,Account newAcct, int key);
	void printAccountsRec(DNode *Current) const;

//	DNode* Assign(Account newAcct);

	bool removeRec(int disc, DNode *&Current, DNode*& removed);

	DNode* retrieveRec(DNode *Current, int disc);

	//used in Rebalance
	void TreetoArr(DNode* &node, Account *arr, int &index, int size);
	DNode*& ArrToTree(Account *arr,int size);
	void AddElement(DNode *&root, Account *arr, int  , int ed);
	DNode *& findParent(DNode *&Current, DNode *&Parent, DNode *&Node, int &whichChild);

	void DeepCopy(const DNode *rhs);

	void clearRec(DNode *Current);
