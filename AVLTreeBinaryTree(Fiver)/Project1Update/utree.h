...
private:
	...
	bool insertRec(UNode *Current, Account newAcct);
	void printUsersRec(UNode *Current) const;


	void RotateLeft(UNode *&Current);
	void RotateRight(UNode *&Current);
	void findParent(UNode *Current, UNode *Parent, UNode *Node, int &whichChild, UNode *&Parenttemp);
	
	UNode*& findLeftMax(UNode *Parent, UNode *Current);

	void DeleteRec(UNode *Current, UNode *Parent, string  username, int disc, DNode*& removed);


	bool DeleteDisc(UNode *Node, int disc,DNode*& removed);

	void clearRec(UNode *Current);
	UNode* retrieveRec(UNode *Current, string username);
